...
def CheckMED(inputParameters, op, filepath, data, storesList):
    ...
    with h5py.File(operantPath, 'r') as f: #with is here to close everything up nicely... could just close it though
        ...
        for dset in f[H5group].keys():  #this will loop through all events in group, names dset     
            ...
            if any(f[H5group][dset][()][:]):
                OperantH5_dict[dset] = f[H5group][dset][()][:] #assigns values under key to numpy array in dictionary
    ...
    # JN ADDITION. Since my med names are not the same as my TTL storeslist names I'm making a dictionary to match them that depends on the task. Add as necessary
    # if inputParameters['storeNameSelect'] == 'ASAP':
    #    from ASAPMatching import matchingDict
    # else:
    #    raise Exception('mystery task! check readTevTsq to align storenames and medpc names in the checkMED function!') 
    # NEVER MIND I'M DOING THIS WHEN I MAKE THE H5 FILE
    
    # eventsList_all2 = np.array(list({k: v for k, v in matchingDict.items() if v in eventsList_all}.keys()))
    ...
    def switch_eventNames(eventsList_all, inputString):
        storesListList = eventsList_all.tolist()
        TTLnames = storesListList[0]
        storenameAndh5_names = storesListList[1]

        event_ind = storenameAndh5_names.index(inputString)
        return TTLnames[event_ind]
